...
extern __GlobalConstructorInit_t __init_constructors_array_start[];
extern __GlobalConstructorInit_t __init_constructors_array_end[];
void __InitGlobalConstructors() {
    for (auto fn = __init_constructors_array_start; fn != __init_constructors_array_end; ++fn) {
		terminal_write(" [Global Constructors] - Calling ");
		terminal_write(reinterpret_cast<uint64_t>(*fn));
		terminal_write("\r\n");
        (*fn)();
    }
}
